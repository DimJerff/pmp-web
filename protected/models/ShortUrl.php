...
class ShortUrl extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id, md5, url', 'required'),
			array('id', 'length', 'max'=>20),
			array('md5', 'length', 'max'=>16),
			array('url', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, md5, url', 'safe', 'on'=>'search'),
		);
	}
	...
	public function beforeValidate() {
		...
		if(!isset($this->id)) $this->id = substr(time(),1).mt_rand(10, 99);
		if(!isset($this->md5)) $this->md5 = md5($this->url, true);
