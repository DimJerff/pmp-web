...
class ExcelExtend extends CApplicationComponent{
    ...
    public function setRowSection($item, $itemNames) {
        ...
        foreach($itemNames as $k=>$v) {
            ...
            switch ($v) {
                case 'number':
                    $this->excelSetNumber($this->activeSheet, $char.$this->rowInd, $item[$k]);
                    break;
                case 'percent':
                    $this->excelSetPercent($this->activeSheet, $char.$this->rowInd, $item[$k]);
                    break;
                case 'money':
                    $this->excelSetMoney($this->activeSheet, $char.$this->rowInd, $item[$k]);
                    break;
                default:
                    $this->excelSetCenter($this->activeSheet, $char.$this->rowInd)->SetCellValue($char.$this->rowInd, $item[$k]);
                    break;
            }
            $ascii++;
        }
