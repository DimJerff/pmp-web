 ...
 */
$PHPMailerPath = Yii::getPathOfAlias('application.vendors.phpmailer');
...
class EMailer extends CApplicationComponent
{
	...
	/**
	 * 发送通知邮件
	 * @param $type string NOTICE|ERROR|EXCEPTION
	 * @param $body
	 */
	public function sendNoticeMail($body, $type='NOTICE'){
		$this->_myMailer->Subject = 'dsp-frontend:'.$type.' Info Email';
		$this->_myMailer->Body = date('Y-m-d H:i:s').' ['.$type.'] '.$body;
		$this->_myMailer->clearAllRecipients();
		foreach($this->exceptionEmail as $address){
			$this->_myMailer->addAddress($address);
		}
		$this->_myMailer->send();
		$this->_myMailer->clearAllRecipients();
	}
	...
	public function sendErrorMail($body){
		$this->sendNoticeMail($body, 'ERROR');
	}
	...
	public function sendExceptionMail(Exception $exception){
		$this->sendNoticeMail($exception->getMessage().$exception->getTraceAsString(),'EXCEPTION');
	}
}